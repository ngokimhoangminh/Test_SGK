    ...
    <div class="container">
        ...
        <div class="max-w-7xl mx-auto py-3 sm:px-6 lg:px-0">
            <div class="flex flex-row-reverse pr-px-30">
                <button-action redirect="{{ route('admin.products.create') }}">
                    Thêm
                </button-action>
            </div>
        </div>
                ...
                <tbody>
                    ...
                    <tr id="row_{{ $product->id }}">
                        ...
                        <td class="align-middle">
                            <el-image
                                url="{{ ($product->image == 'default.png') ? asset('libs/uploads/'.$product->image) : asset('storage/'.$product->image) }}"
                                width="140px" height="140px">
                            </el-image>
                        </td>
                        @if( $product->status )
                        <td>
                            <button-action :redirect="`{{ route('admin.products.active', $product->id) }}`">
                                Hiển Thị
                            </button-action>
                        </td>
                        @else
                        <td>
                            <button-action class-name="btn btn-danger"
                                :redirect="`{{ route('admin.products.active', $product->id) }}`">
                                Ẩn
                            </button-action>
                        </td>
                        @endif
                        <td>
                            <button-action class-name="btn btn-success"
                                :redirect="`{{ route('admin.products.edit', $product) }}`"><i class="fas fa-edit"></i>
                            </button-action>
                            <confirm-delete name="{{ $product->name }}" :id="{{ $product->id }}"
